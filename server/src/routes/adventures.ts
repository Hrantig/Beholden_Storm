...
const CombatantImport = z.object({
  ...
  focusCurrent: z.number().nullable().default(null),
  focusMax: z.number().nullable().default(null),
  investitureCurrent: z.number().nullable().default(null),
  investitureMax: z.number().nullable().default(null),
    ...
    const encounters = encRows.map((encRow) => {
      ...
      const combatants = (combatantsByEnc.get(enc.id) ?? []).map((c) => {
        ...
        return {
          ...
          focusCurrent: combatant.focusCurrent,
          focusMax: combatant.focusMax,
          investitureCurrent: combatant.investitureCurrent || null,
          investitureMax: combatant.investitureMax || null,
          ac: combatant.ac,
    ...
    db.transaction(() => {
      ...
      for (const [i, enc] of imp.encounters.entries()) {
        ...
        for (const [ci, c] of enc.combatants.entries()) {
          ...
          focusCurrent: c.focusCurrent,
          focusMax: c.focusMax,
          investitureCurrent: c.investitureCurrent || null,
          investitureMax: c.investitureMax || null,
          ac: c.ac,
