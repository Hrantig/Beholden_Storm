...
export function CharacterHudPanel(props: CharacterHudPanelProps) {
  ...
  const identityFields = [
    ...
    { label: "Age", value: char.characterData?.age },
    { label: "Height", value: char.characterData?.height },
    { label: "Weight", value: char.characterData?.weight },
    { label: "Hair", value: char.characterData?.hair },
    { label: "Skin", value: char.characterData?.skin },
  ].filter((field): field is { label: string; value: string } => typeof field.value === "string" && field.value.trim().length > 0);
  ...
  return (
    <>
            ...
            {identityFields.length > 0 && (
              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 4 }}>
                {identityFields.map((field) => (
                  ...
                  >
                    <span style={{ color: "rgba(160,180,220,0.6)", marginRight: 4 }}>{field.label}</span>
                    {field.value}
                  </span>
                ))}
              </div>
      ...
      {condPickerOpen && (
        <>
          <div onClick={() => setCondPickerOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 900, background: "rgba(0,0,0,0.45)" }} />
          <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, zIndex: 901, width: "min(340px, 90vw)", background: "#0e1220", borderLeft: "1px solid rgba(255,255,255,0.12)", display: "flex", flexDirection: "column", boxShadow: "-8px 0 30px rgba(0,0,0,0.5)" }}>
