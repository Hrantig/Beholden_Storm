...
import { abilityNamesToKeys, parseSkillList } from "@/views/CharacterCreatorUtils";
import { NavButtons, SpellPicker } from "@/views/CharacterCreatorParts";
import {
  detailBoxStyle,
  headingStyle,
  inputStyle,
...
export function renderSpeciesStep({
  ...
  const main = (
    <div>
      ...
      <NavButtons step={2} onBack={onBack} onNext={onNext} nextDisabled={!selectedRaceId} />
    </div>
  );
  ...
  const main = (
    <div>
      ...
      {isCaster && cantripCount > 0 && (
        <SpellPicker title="Cantrips" spells={classCantrips} chosen={chosenCantrips} max={cantripCount} emptyMsg="No cantrips found in compendium for this class." onToggle={toggleCantrip} />
      )}
